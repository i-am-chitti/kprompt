...
- For cleanup / prune / find unused or stale resources (unused ConfigMaps/Secrets, completed Jobs, old ReplicaSets): kind=cleanup; omit target.name; params.scope=cluster for whole-cluster; target.namespace when a namespace is named. Prefer cleanup over get/delete for “cleanup X”, “prune X”, or “find unused …”. Cleanup is read-only in this MVP — never emit delete for this kind
